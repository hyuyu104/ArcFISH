...
def simulate_mc(nstate:int, p:int, m:float=10) -> np.array:
    ...
    m : float
        ...
        `P(stay) = mP(leave)`, default to 5.

    Returns
    -------
    (p,) np.array
        Simulated Markov chain filled with integers ranging from 0 to
        nstate - 1.
    """
    prob = 1/(nstate + m - 1)
    P = np.ones((nstate, nstate))*prob
    P += np.identity(nstate)*prob*(m-1)
...
class ABSample:
    """Generate A/B compartment simulations by SBM.
    ...
    rho : float
        Float between 0 and 1, sparsity parameter.
    B : (s,s) or (3,s,s) np.ndarray
        If of shape `(s,s)`, all axes will be generated by the same `B`.
        If of shape `(3,s,s)`, different axes will be generated from
        different distributions specified by `B`.
    """
    def __init__(
        self,
        p:int, 
        ...
        self.comm = simulate_mc(B.shape[-1], p)
        self.C = pd.get_dummies(self.comm).values.astype("int")
        
        if len(B.shape) == 2:
            self.samples = np.stack([simulate_sbm(
                self.comm, rho, B, n
            ) for _ in range(3)]).transpose((1, 0, 2, 3))
        if len(B.shape) == 3:
            self.samples = np.stack([simulate_sbm(
                self.comm, rho, B[i], n
            ) for i in range(B.shape[0])]).transpose((1, 0, 2, 3))
